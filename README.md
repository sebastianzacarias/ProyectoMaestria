...
Plataforma avanzada de análisis biomecánico para tenis basada en Inteligencia Artificial. Este sistema utiliza visión por computadora (**YOLOv26**) y modelos de large-lenguaje (**llama3.2:3b**) para proporcionar un análisis detallado de la técnica del jugador, identificando golpes, midiendo ángulos articulares y ofreciendo recomendaciones profesionales de entrenamiento.
...
*   **Estimación de Pose:** Extracción de puntos clave (landmarks) del cuerpo humano utilizando **YOLOv26-Pose** (modelo `yolo26x-pose.pt`).
...
*   **Visión por Computadora:** Ultralytics YOLOv8 (Detección de Objetos: `yolo26x.pt`, Pose-Estimation: `yolo26x-pose.pt`)
...
2.  **Ollama instalado** y ejecutando el LLM:
    ```bash
